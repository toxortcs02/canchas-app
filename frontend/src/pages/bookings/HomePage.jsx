...
import EditParticipantsModal from '../../components/EditParticipantsModal.jsx';
import './HomePage.css';
...
const HomePage = () => {
  ...
  // 🆕 Handler para click en reserva
  const handleBookingClick = (booking, court) => {
    // Verificar que sea el creador
    if (booking.created_by === loggedUserId) {
      // Preparar datos para el modal (ya viene todo en booking)
      const bookingData = {
        id: booking.id,
      ...
      alert("Solo el creador de la reserva puede modificar los participantes");
    }
  };
  ...
  // 🆕 Callback cuando se actualiza con éxito
  ...
  const renderCell = (timeSlot, court) => {
    ...
    if (booking) {
      ...
      const isOwner = booking.created_by === loggedUserId;
      
      return (
        <td 
          key={`${timeSlot}-${court.id}`} 
          className={`booking-cell occupied ${isOwner ? 'clickable owner' : ''}`}
          rowSpan={booking.duration_blocks}
          onClick={() => isOwner && handleBookingClick(booking, court)}
          style={{ cursor: isOwner ? 'pointer' : 'default' }}
          title={isOwner ? 'Click para editar participantes' : ''}
        >
          <div className="booking-content">
            <div className="booking-creator">
              <strong>{booking.first_name} {booking.last_name}</strong>
              {isOwner && <span className="owner-badge">✏️</span>}
            </div>
  ...
  return (
    <div className="home-page">
      ...
      {/* 🆕 Modal de edición de participantes */}
      {selectedBooking && (
